...
void TextOverlay::draw(const std::string & /*text*/, int screenWidth, int screenHeight) const {
    if (!shader_) {
        return;
    }

    const float panelW = 420.0f / static_cast<float>(screenWidth) * 2.0f;
    const float panelH = 120.0f / static_cast<float>(screenHeight) * 2.0f;
